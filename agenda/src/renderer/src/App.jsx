        ...
        <main className={`flex-1 p-4 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-0'}`}>
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex space-x-4">
              <button onClick={() => setView('day')} className={`${view === 'day' ? 'font-semibold' : ''}`}>Día</button>
              <button onClick={() => setView('workWeek')} className={`${view === 'workWeek' ? 'font-semibold' : ''}`}>Semana laboral</button>
              <button onClick={() => setView('week')} className={`${view === 'week' ? 'font-semibold' : ''}`}>Semana</button>
              <button onClick={() => setView('month')} className={`${view === 'month' ? 'font-semibold' : ''}`}>Mes</button>
            </div>
          </div>
          ...
          {view === 'month' && (
            <div>
              ...
              <button onClick={() => setShowForm(true)} className="p-2 bg-blue-500 text-white rounded mt-4">
                Agregar Tarea
              </button>
            </div>
          )}
