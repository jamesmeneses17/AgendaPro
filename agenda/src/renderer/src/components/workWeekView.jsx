...
function WorkWeekView({ date, setDate }) {
  ...
  const startDate = startOfWeek(date, { weekStartsOn: 1 }); // Empieza el lunes
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-6 border border-gray-300 h-[calc(100vh-200px)] overflow-y-auto">
        ...
        <div className="bg-gray-100 text-center sticky top-0 z-10">
          <div className="py-6 border-b text-sm text-black"> {/* Cambiado a negro */}
            Horas
          </div>
          {hours.map((hour) => (
            <div
              key={hour}
              className="h-12 border-b flex items-center justify-center text-black" // Cambiado a negro
            >
              {`${hour}:00`}
            </div>
          ))}
        ...
        {days.map((day, dayIndex) => (
          <div key={dayIndex} className="text-center">
            ...
            <div className="bg-gray-100 text-black font-semibold py-2 sticky top-0 z-10 border-b"> {/* Cambiado a negro */}
